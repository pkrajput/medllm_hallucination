...
def hallucination(reference, ground_truth):
    ...
    try:
        questions = mqag_model.generate(context=reference, do_sample=True, num_questions=3, seed = 42)
        hallucinate = 0
        hallucinate_ground = 0
        for j, question_item in enumerate(questions):
            Q = question_item["question"]
            options = [question_item["options"][0], question_item["options"][1], question_item["options"][2], "None of the above"]
            probs = mqag_model.answer(questions = [{"question": Q, "options": options}], context = reference)
            ground_truth_probs = mqag_model.answer(questions = [{"question": Q, "options": options}], context = ground_truth)
...
for i, response_column in tqdm(enumerate(response_columns, start = 1), desc = "Processing"):
    
    new_column_name = f"hallucination in {i}"
    df[new_column_name] = df.progress_apply(lambda row: hallucination(row[response_column], row["output"]), axis = 1)
